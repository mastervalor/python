...
class Users:
    ...
    def get_language_by_users(cls, data):
        ...
        for row in results:
            ...
            language.for_users.append(cls(user_data))
        return language
    ...
    def check_email(cls,email):
        query = "SELECT * from users where email = %(email)s"
        results = connectToMySQL('login').query_db(query,email)
        if len(results) < 1:
            return False
        return cls(results[0])
    ...
    def validate_user(user):
        is_valid = True
        if len(user['first_name']) <= 0 or len(user['last_name']) <= 0 or len(user['email']) <= 0 or len(user['password']) <= 0 or len(user['birthdate']) <= 0 or len(user['age']) <= 0 or len(user['location']) <= 0:
            is_valid = False
